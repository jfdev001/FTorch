...
configured, built, *and* installed FTorch using CMake commands like `cmake [args...]`, 
`cmake --build [args...]`, *and* `cmake --install [args...]`, respectively. 
...
If you do not wish to specify the entire path to that particular FTorch
installation pkgconfig directory during every invocation of `pkg-config`, you
can update the `PKG_CONFIG_PATH` environment variable (e.g., in `~/.bashrc`)
accordingly. Then, you only have to specify `ftorch` as an argument to
`pkg-config`:
```sh
